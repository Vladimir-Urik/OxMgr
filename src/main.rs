...
async fn run_cli_command(command: Commands, config: &AppConfig) -> Result<()> {
    ...
    match command {
        ...
        Commands::Service { command, system } => {
            service_command(command, system, config)?;
        }
        Commands::Daemon {
            command: DaemonCommand::Run,
...
fn service_command(command: ServiceCommand, system: InitSystem, config: &AppConfig) -> Result<()> {
    let executable = std::env::current_exe().context("failed to resolve current executable")?;
    let resolved = match system {
        InitSystem::Auto => {
            if cfg!(target_os = "macos") {
...
mod tests {
    use super::{
        plan_apply_actions, render_launchd_plist, render_systemd_service, validate_oxfile_command,
        validate_resolved_specs, ApplyAction, DesiredProcessSpec,
    };
    use crate::config::AppConfig;
